...
if( !empty($this->results['lists']) ){
    ...
    foreach ($this->results['lists'] as $i => $item) { 
        ...
        $name = '<a href="'.URL.'agent/member/'.$item['m_id'].'" class="fwb">'.$item['name'].'</a>
				<div class="mts fsm">'. 
					( !empty($item['email']) 
		...
		$actions = $this->ui->toggle()
			...
			->option('เปลี่ยนรหัสผ่าน')
				->link(URL."agent/member/change_password/{$item['m_id']}")
			...
			$actions
				...
				->option('ลบ')
					->link( URL."agent/member/del/{$item['m_id']}")
					->attr('data-plugins', 'dialog');
        ...
        $tr .= '<tr class="'.$cls.'" data-id="'.$item['m_id'].'">'.
            ...
            '<td class="actions">'.
            	...
            	'<span class="group-btn" style="width:130px;">'.	
            			'<a class="btn" href="'.URL.'agent/member/edit/'.$item['m_id'].'" data-plugins="dialog"><i class="icon-pencil mrs"></i>แก้ไข</a>'.
